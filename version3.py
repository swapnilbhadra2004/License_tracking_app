...
with tab2:
    ...
    with col1:
        ...
        direction = st.radio("Select Direction:", ["Entry", "Exit"], horizontal=True)
        ...
        if st.button("🚀 Check Plate", use_container_width=True):
            if test_plate:
                ...
                if has_access:
                    st.success(f"✅ ACCESS GRANTED - {direction} Allowed")
                    st.balloons()
                else:
                    st.error(f"❌ ACCESS DENIED - {direction} Blocked (Unknown Vehicle)")
...
with tab4:
    ...
    if st.session_state.plate_history:
        ...
        if 'direction' not in history_df.columns:
            history_df['direction'] = 'Entry'

        filtered_history = history_df[
            (history_df['access'].isin(filter_access)) &
            (history_df['direction'].isin(filter_direction))
        ]
